...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Has comprado el ~y~" .. tattooName .. "~s~ tattoo por ~g~$" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "No tienes suficiente dinero para este tattoo")
		cb(false)
	end
end)
