...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Tienda Tattoo")
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Has eliminado el ~y~" .. GetLabelText(label) .. "~s~ tattoo")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Abrir Tienda Tattoo", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Cambiar vista de camara", 21, "Cambiar opacidad", {90, 89}, "Comprar / quitar", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Tienda Tattoo", function()
